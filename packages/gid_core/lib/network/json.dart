...
/// Decode a JSON object that arrived as a STRING rather than as parsed data.
///
/// FCM data messages are `Map<String, String>` on every platform -- the payload
/// has to be encoded to get through, and comes back out as text. Returns null
/// rather than throwing: a malformed push is a delivery problem, not a reason
/// to take the app down.
Json? decodeJsonObject(String source) {
  try {
