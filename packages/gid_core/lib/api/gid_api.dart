...
class GidApi {
  ...
  ///
  /// [role] is a SIGNUP hint and nothing more: it decides what a brand new
  /// account is created as, and is ignored for an account that already exists.
  /// The worker app must pass `worker` — Google hands over no way for the person
  /// to say which side of the platform they are on, and an account created as a
  /// customer cannot reach a single worker route.
  Future<AuthSession> signInWithGoogle(String idToken, {String role = 'customer'}) async {
  ...
  /// Turn the signed-in account into a worker account.
  ///
  /// The escape hatch for someone who signed up on the wrong side of the
  /// platform — most often through Google, which until now always produced a
  /// customer account. Idempotent when the account is already a worker; the
  /// backend refuses an account that has already booked work (409) and any
  /// operator account (403).
  Future<AppUser> becomeWorker() async {
    final json = await _client.post('/auth/role/worker');
  ...
  ///
  /// `appVersion` is REQUIRED by the backend (deviceTokenSchema in
  /// routes/notifications.ts); omitting it made every registration a 400, which
  /// went unnoticed because nothing called this method at all.
