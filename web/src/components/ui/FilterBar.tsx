...
export function FilterBar({ filters, onChange, config, className, showActiveCount = true }: FilterBarProps) {
  ...
  const filterRefs = useRef<Record<string, HTMLDivElement>>({})
  ...
  return (
    <div className={cn("flex flex-wrap items-center gap-3 p-3 bg-ink rounded-lg border border-muted/20", className)}>
      <div className="flex items-center gap-2 text-sm text-muted">
        <Funnel size={16} weight="regular" aria-hidden="true" />
        <span>Filters</span>
        {showActiveCount && activeCount > 0 && (
          <span className="px-2 py-0.5 bg-accent/15 text-accent text-xs font-medium rounded-full">{activeCount}</span>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 flex-1 min-w-0">
            ...
            return (
              <div key={filter.key} className="relative">
                <input
                  ...
                  className={cn(
                    "w-48 sm:w-64 px-3 py-1.5 bg-muted/10 border border-muted/20 rounded-lg text-sm text-ink placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent",
            ...
            return (
              <div key={filter.key} className="relative" ref={(el) => (filterRefs.current[filter.key] = el!)}>
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1.5 px-3 py-1.5 bg-muted/10 border border-muted/20 rounded-lg text-sm text-ink hover:bg-muted/20 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent",
                ...
                >
                  ...
                  <ChevronDown size={12} weight="regular" aria-hidden="true" />
                </button>

                {openFilter === filter.key && (
                  <div
                    className="absolute right-0 top-full mt-1.5 w-56 bg-ink border border-muted/20 rounded-lg shadow-lg py-1 z-50 max-h-60 overflow-auto scrollbar-thin"
                    role="listbox"
                    aria-label={filter.label}
                  >
                    {filter.options?.map((option) => (
                      ...
                      >
                        {filter.multiple && (
                          <span className={cn("w-4 h-4 rounded border-2 flex items-center justify-center", (Array.isArray(value) ? value.includes(option.value) : false) ? "bg-accent border-accent text-ink" : "border-muted/30")}>
                    ...
                    {filter.multiple && value && Array.isArray(value) && value.length > 0 && (
                      <>
            ...
            return (
              <div key={filter.key} className="relative">
                <input
                  ...
                  className={cn(
                    "w-40 px-3 py-1.5 bg-muted/10 border border-muted/20 rounded-lg text-sm text-ink focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent",
                    hasValue && "border-accent/50"
                  )}
                  aria-label={filter.label}
                />
            ...
            return (
              <div key={filter.key} className="flex items-center gap-1">
                <input
                  ...
                  placeholder="From"
                  className={cn("w-36 px-3 py-1.5 bg-muted/10 border border-muted/20 rounded-lg text-sm text-ink focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent", hasRange && "border-accent/50")}
        ...
        {activeCount > 0 && (
          <button
            ...
            className="px-3 py-1.5 text-sm text-crit hover:text-crit/70 hover:bg-crit/10 rounded-lg transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-crit"
          >
            Clear all
          </button>
        )}
